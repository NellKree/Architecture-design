...
2. **Просмотр характеристик аккаунта**:
   - Пользователь запрашивает информацию о своем аккаунте. UI передает запрос контроллеру (ViewModel).
   - Контроллер запрашивает данные аккаунта у модели (Model).
   - Модель отправляет запрос на сервер для получения данных аккаунта.
...
3. **Просмотр отчёта**:
   ...
   - Контроллер обращается к модели за данными отчета, которая затем запрашивает их у сервера.
   - Сервер возвращает данные отчета, которые передаются обратно через модель и контроллер в UI.
...
4. **Запуск анализа бот-активности**:
   ...
   - Контроллер отправляет данные в модель, которая в свою очередь передает их на сервер для обработки.
   - Сервер возвращает результаты анализа бот-активности, которые передаются обратно через модель и контроллер в UI.
