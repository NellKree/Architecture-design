...
![Диаграмма системного контекста](./DP.png)

#### Пояснения к диаграмме последовательности

1. **Открытие приложения**:
   - Пользователь запускает приложение, инициируя взаимодействие с пользовательским интерфейсом (UI).

2. **Просмотр характеристик аккаунта**:
   - Пользователь запрашивает информацию о своем аккаунте. UI передает запрос контроллеру.
   - Контроллер запрашивает данные аккаунта у коннектора.
   - Коннектор отправляет запрос на сервер для получения данных аккаунта.
   - Сервер возвращает данные аккаунта, которые передаются обратно через модель и контроллер в пользовательский интерфейс.
   - UI отображает характеристики аккаунта пользователю.

3. **Просмотр отчёта**:
   - Пользователь запрашивает отчет. UI передает этот запрос контроллеру.
   - Контроллер обращается к коннектору за данными отчета, который затем запрашивает их у сервера.
   - Сервер возвращает данные отчета, которые передаются обратно через коннектор и контроллер в UI.
   - UI отображает отчет пользователю.

4. **Запуск анализа бот-активности**:
   - Пользователь инициирует анализ бот-активности, вводя необходимые данные через UI.
   - UI передает данные для анализа контроллеру.
   - Контроллер отправляет данные в коннектор, который в свою очередь передает их на сервер для обработки.
   - Сервер возвращает результаты анализа бот-активности, которые передаются обратно через коннектор и контроллер в UI.
   - UI отображает результаты анализа пользователю.

5. **Закрытие приложения**:
   - Пользователь завершает работу с приложением, закрывая его.
...
participant "Контроллер" as ViewModel
participant "Коннектор к серверу" as Model
database "Сервер" as Server

a-> UI: Открытие приложения

activate UI
a-> UI: Просмотр характеристик аккаунта
UI-> ViewModel: Выбор аккаунта

activate ViewModel
ViewModel -> Model: Запрос данных аккаунта

activate Model
Model -> Server: Запрос данных аккаунта

activate Server
Server --> Model: Возврат данных аккаунта
deactivate Server

Model --> ViewModel: Возврат данных аккаунта
deactivate Model

ViewModel --> UI: Возврат аккаунта
deactivate ViewModel

UI --> a: Отображение характеристик аккаунта


a-> UI: Просмотр отчёта
UI-> ViewModel: Выбор отчёта

activate ViewModel
ViewModel -> Model: Запрос данных отчёта

activate Model
Model -> Server: Запрос данных отчёта

activate Server
Server --> Model: Возврат данных отчёта
deactivate Server

Model --> ViewModel: Возврат данных отчёта
deactivate Model

ViewModel --> UI: Возврат отчёта
deactivate ViewModel

UI --> a: Отображение отчёта




a-> UI: Запуск анализа бот-активности
 
UI -> ViewModel: Ввод данных для анализа
 
activate ViewModel
ViewModel -> Model: Передача данных для анализа

activate Model
Model -> Server: Передача данных для анализа

activate Server
Server --> Model: Возврат результатов анализа бот-активности
deactivate Server

Model --> ViewModel: Возврат результатов анализа бот-активности
deactivate Model

ViewModel --> UI: Возврат результатов анализа бот-активности
deactivate ViewModel

UI--> a: Показ результатов анализа бот-активности

a-> UI: Закрытие приложения
@enduml
```
### Модель базы данных
